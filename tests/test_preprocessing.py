...
def test_collapse_duplicate_genes_normalizes_case_before_grouping() -> None:
    df = pd.DataFrame(
        {
            "genes": ["Prkaca", "PRKACA", "Btk"],
            "group1": [1.0, 5.0, 2.0],
...
def test_collapse_duplicate_genes_handles_all_missing_duplicate_group() -> None:
    df = pd.DataFrame(
        {
            "genes": ["PRKACA", "PRKACA", "BTK"],
            "group1": [np.nan, np.nan, 1.0],
            "group2": [np.nan, np.nan, 1.0],
            "group3": [np.nan, np.nan, 1.0],
            "group4": [np.nan, np.nan, 1.0],
            "group5": [np.nan, np.nan, 1.0],
            "group6": [np.nan, np.nan, 1.0],
        }
    )
...
def test_correct_phospho_to_protein_rejects_non_unique_total_gene_keys() -> None:
    phospho = pd.DataFrame(
        {
            "gene_names": ["PRKACA"],
            "p_group1": [8.0],
    ...
    try:
        ...
        assert "unique gene keys" in str(exc)
