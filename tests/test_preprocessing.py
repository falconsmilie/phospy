...
def test_collapse_duplicate_genes_breaks_equal_mean_ties_deterministically_by_uid() -> (
    None
):
    df = pd.DataFrame(
        {
            "uid": ["z-row", "a-row", "btk-row"],
            "genes": ["Prkaca", "Prkaca", "Btk"],
            "group1": [5.0, 5.0, 2.0],
            "group2": [5.0, 5.0, 2.0],
        }
    ...
    prkaca = out.loc[out["genes"] == "PRKACA"].iloc[0]
    assert prkaca["uid"] == "a-row"


def test_collapse_duplicate_genes_breaks_equal_mean_ties_by_original_order_without_uid() -> (
    None
):
    df = pd.DataFrame(
        {
