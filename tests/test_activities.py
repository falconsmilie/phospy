...
def test_compute_ksea_scores_returns_counts_only_for_scored_kinases() -> None:
    pred_mat = pd.DataFrame(
        {
            "PRKACA": [0.9, 0.8, 0.7, 0.1],
            "BTK": [0.95, 0.85, 0.1, 0.75],
        },
        index=["A;S1;", "B;S2;", "C;S3;", "D;S4;"],
    )
    phospho_matrix = pd.DataFrame(
    ...
    assert list(scores.index) == ["PRKACA"]
    assert list(counts.index) == ["PRKACA"]
    assert int(counts.loc["PRKACA"]) == 3


def test_compute_ksea_scores_returns_empty_counts_when_no_kinases_pass_threshold() -> (
    None
):
    pred_mat = pd.DataFrame(
        {
    ...
    assert scores.empty
    assert counts.empty
    assert list(scores.columns) == ["phospho_corrected_1", "phospho_corrected_2"]
    assert counts.name == "n_substrates"
    assert counts.index.name == "kinase"
