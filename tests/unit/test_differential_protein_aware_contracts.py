...
def _protein_aware_status_table() -> pd.DataFrame:
    site_index = protein_site_key_index(
        protein_identifiers=("MAPK14", "AKT1", "GSK3B", "PRKACA", "RPS6", "EIF4EBP1"),
        sites=("Y182", "T308", "S9", "S339", "S235", "T37"),
    )
    statuses = [
        DIFFERENTIAL_RESULT_STATUS_TESTED,
        DIFFERENTIAL_RESULT_STATUS_WITHHELD_PROTEIN_PREPARATION_INELIGIBLE,
    ...
    table = pd.DataFrame(
        {
            ...
            **site_key_context_columns(site_index),
            "gene_symbol": ["MAPK14", "AKT1", "GSK3B", "PRKACA", "RPS6", "EIF4EBP1"],
            "site": ["Y182", "T308", "S9", "S339", "S235", "T37"],
            "logFC": [1.0, np.nan, np.nan, np.nan, np.nan, np.nan],
            "t": [2.0, np.nan, np.nan, np.nan, np.nan, np.nan],
            "P.Value": [0.04, np.nan, np.nan, np.nan, np.nan, np.nan],
            "adj.P.Val": [0.04, np.nan, np.nan, np.nan, np.nan, np.nan],
