...
def test_signalome_bundle_site_metadata_rejects_conflicting_grouping_alias() -> None:
    table = pd.DataFrame(
        {
            "display_id": ["MAPK14;Y182;"],
            "protein_group_id": ["GROUP_A"],
            "protein_id": ["GROUP_B"],
        },
        index=pd.Index(["test.site_key.row_a"], name="site_key"),
    )
    ...
    with pytest.raises(PhosPyInputError, match="conflicting Signalome grouping"):
        _normalize_site_metadata_for_dataset_contract(table)
