...
import phospy.signalomes.clustering.exact_python as exact_backend_exports
import phospy.signalomes.clustering.orchestration as orchestration_exports
from phospy.signalomes.clustering.models import SignalomeClusteringEngineRequest
...
def test_candidate_scoring_policy_boundary_does_not_use_arg_type_ignore() -> None:
    ...
    adapter_source = (
        root / "src" / "phospy" / "signalomes" / "clustering" / "tree_engine_adapter.py"
    ).read_text(encoding="utf-8")
    orchestration_source = (
        root / "src" / "phospy" / "signalomes" / "clustering" / "orchestration.py"
    ).read_text(encoding="utf-8")
