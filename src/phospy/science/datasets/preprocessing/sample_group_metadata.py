"""Resolve sample groups from dataset preprocessing metadata."""
...
class ResolvedSampleGroups:
    ...
    def group_labels(self) -> tuple[str, ...]:
        """Return distinct group labels in phospho sample order."""

        return tuple(self.sample_order_by_group.keys())


class SampleGroupMetadataResolver:
    """Validate and resolve sample groups for preprocessing consumers."""
        ...
        *,
        ...
        if sample_metadata is None:
            raise PhosPyInputError(
                "dataset preprocessing sample-group metadata resolution requires "
                "sample_metadata input data"
            )
        if group_column is None or not str(group_column).strip():
            raise PhosPyInputError(
                "dataset preprocessing sample-group metadata resolution "
...
def _normalize_label_index(index: pd.Index, *, field_name: str) -> pd.Index:
    ...
    if missing_positions:
        raise PhosPyInputError(
            f"{field_name} must not contain missing labels for sample-group "
            f"metadata resolution; found positions: "
            f"{_format_positions(missing_positions)}"
        )
    if blank_positions:
        raise PhosPyInputError(
            f"{field_name} must not contain blank labels for sample-group metadata "
            f"resolution; found positions: {_format_positions(blank_positions)}"
        )
...
def _require_unique_labels(index: pd.Index, *, field_name: str) -> None:
    ...
    raise PhosPyInputError(
        f"{field_name} contains duplicate sample labels for sample-group metadata "
        f"resolution: {preview}{suffix}"
    )
...
def _require_column(sample_metadata: pd.DataFrame, *, column: str) -> None:
    ...
    if len(matches) > 1:
        raise PhosPyInputError(
            "dataset preprocessing sample-group metadata resolution group_column "
            f"resolves to duplicate sample_metadata column {column!r}"
        )
    raise PhosPyInputError(
        "dataset preprocessing sample-group metadata resolution group_column "
        f"references missing sample_metadata column {column!r}"
    )
...
__all__ = ["ResolvedSampleGroups", "SampleGroupMetadataResolver"]
