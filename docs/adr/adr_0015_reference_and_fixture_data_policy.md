...
- record fixture-family provenance and classification when checked in as a
  release-validation family rather than as an inline test-only literal
...
Release-validation parity fixtures that are checked in as standalone fixture
families should include a manifest recording classification, external
implementation when applicable, generator path, command, seed, timestamp/source
policy, generator hash, and file hashes.
...
Expected outputs generated by PhosPy itself are never external parity
authority. Those fixtures must be labelled as regression, golden, or
performance inputs according to their purpose.

## Release-Validation Fixture Families

Current release-validation fixture families include:
...
- `tests/fixtures/release_validation_regression/kinase_sparse_support/`:
  PhosPy regression fixtures for substrate support floors, sparse overlap,
  self-inclusion policy, localisation attrition, production threshold failure,
  and ssGSEA permutation-significance status.
- `tests/fixtures/release_validation_regression/signalome_safety/`: PhosPy
  regression fixtures for small-sample network thresholds, missing-dimension
  handling, clustering invariance, and historical threshold-2 reconstruction.
