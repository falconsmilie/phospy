...
External parity must use an authoritative external implementation or fixture
source. If expected values are generated by PhosPy, the case is a PhosPy
regression, golden, or performance contract, not parity.

## Amendment: Scientific Validation Evidence Categories

**Date:** 2026-08-08

PhosPy now maintains a release-grade validation evidence matrix in
`docs/scientific-coverage.md`. The matrix records, per method, the estimand,
expected inputs, known-truth or external reference, comparison method,
acceptance threshold, sensitivity analysis, fixture provenance, evidence
category, and supported interpretation.

Evidence categories are distinct review concepts:

- `regression`: PhosPy-owned expected values or structural contracts that catch
  implementation drift.
- `parity`: independently generated external outputs whose semantics genuinely
  match the compared PhosPy output.
- `synthetic_validation`: planted or closed-form known-truth evidence generated
  outside the exact production code path being validated.
- `empirical_validation`: independent real-world empirical evidence with a
  named dataset/source and documented comparison result.

PhosPy-generated expected values must not be relabelled as external parity.
Synthetic known-truth fixtures must not be relabelled as empirical validation.
Benchmarks are scale evidence, not scientific parity. Fixture thresholds are
release validation gates and must not become hidden parameter-tuning targets.
...
- empirical-Bayes modes explicitly exposed by contract (`standard`, `robust`,
  optional `trend`)
- a large-feature R/limma trend fixture when the fixture manifest identifies
  limma as the external authority, records the generator, command, seed,
  versions, and file hashes, and compares only scientifically equivalent
  quantities
...
Adverse fixed-effect or numerical-design cases without external expected values
are release-gated PhosPy regression contracts, not limma or PhosR parity.
...
Release validation should run the authoritative aggregate command
(`make release-check`) so parity is evaluated together with normal
unit/integration, release/golden/reproducibility, and performance contracts.

Within that release gate, threshold-bearing parity failures are release-blocking.
Informational `parity_diagnostic` checks remain non-threshold-bearing and are
excluded from the blocking parity target.
