...
optional replicate metadata for `sps_ruv_style` or required active replicate
metadata for `ruv_iii_style`, explicit `CorrectionMissingnessPolicy`,
`n_unwanted_factors`, diagnostics, and provenance. The two methods keep distinct
protected-design and replicate semantics and apply correction before downstream
workflows consume the analysis-ready matrix. They are not PhosR-equivalent
SPS/RUV-III parity, ComBat, limma `removeBatchEffect`, hidden online control
fetching, or implicit control selection.
