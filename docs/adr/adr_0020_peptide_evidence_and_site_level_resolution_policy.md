...
- production-path synthetic ground-truth fixtures covering unit, equal
  fractional, unequal explicit fractional, duplicate, missing-value,
  mixed ambiguous/unambiguous, contradictory-localisation,
  row-order-invariance, and log2 unit-mapping round-trip cases; and
...
## Amendment: Release Validation Bias Regimes

**Date:** 2026-08-09

Peptide-to-site validation now includes a production-path synthetic known-truth
fixture under
`tests/fixtures/release_validation_regression/peptide_site_bias_regimes/`.
The fixture contains raw peptide-evidence rows, explicit peptide-to-site
mappings, policy/scale assumptions, declared site-level known truth,
independently generated expected estimator output, and signed/absolute bias.
Release tests execute the raw fixture through
`PeptideEvidenceDatasetResolver` and compare the production result to the
independent expected site-estimate matrix within the declared tolerance.

The validated estimator is the ADR-0020 sample-level policy:
linear peptide-abundance evidence is multiplied by explicit mapping fractions,
then finite allocated values are summarised by arithmetic mean per resolved
site and sample. The fixture covers duplicate concordance and discordance,
equal and unequal ambiguous allocation, a missing value in one sample, an
entirely missing peptide observation, localisation assignment to the wrong
site, and row-order invariance.

This fixture is `synthetic_validation`, not external parity and not empirical
validation. It is not biological ground truth. Resolver expected outputs
generated by PhosPy remain regression contracts. The fixture exists to expose
adverse-regime bias and interpretation limits; production allocation
parameters must not be silently tuned against it.
