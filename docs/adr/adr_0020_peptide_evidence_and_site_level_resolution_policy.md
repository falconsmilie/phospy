...
Peptide-to-site validation now includes a closed-form synthetic bias fixture
under
`tests/fixtures/release_validation_regression/peptide_site_bias_regimes/`.
It quantifies duplicate discordance, ambiguous equal splitting, missing peptide
evidence, and localisation-error signal transfer against explicit known truth.

This fixture is `synthetic_validation`, not external parity and not empirical
validation. Resolver expected outputs generated by PhosPy remain regression
contracts. The fixture exists to expose adverse-regime bias and interpretation
limits; production allocation parameters must not be silently tuned against it.
