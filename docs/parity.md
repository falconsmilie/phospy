...
## SPS and RUV-III External Reference Evidence

The fixture family at `tests/fixtures/rewrite_parity/phosr_ruv/` provides a
narrow, pinned external comparison for multi-reference SPS discovery and the
finite-`k` RUV-III kernel. It was generated by R 4.5.2, PhosR 1.13.1 at commit
`1be74902b775833c64f5833e70538eaf843cf6a5`, and `ruv` 0.9.7.1. Exact direct
package versions, the generation environment, redistribution status,
comparison tolerances, intermediate SPS scores, and corrected RUV-III matrices
are recorded with the fixture. Normal tests consume only committed files; they
do not start R, install packages, access the network, or regenerate outputs.
PhosPy has no runtime dependency on R.

The SPS evidence begins with synthetic absolute processed log2 abundance and
makes preparation explicit. Within each reference dataset and site, the mean
of two baseline replicates is subtracted from every sample. The resulting
condition-relative log2 matrix has a zero baseline-condition mean and is the
identical matrix supplied to both PhosR `getSPS` and PhosPy. Absolute abundance
is retained only as auditable source evidence; it is not passed directly to
PhosPy SPS discovery. The fixture covers three independent references,
replicated conditions, deliberately different row orders, stable and strongly
changing sites, consensus ranking, top-N selection, and ties. PhosPy's
site-key-ascending tie order remains its deterministic contract; an external
...
replicate-set effects, and a sample-order permutation. Corrected matrices and
rank/singular-value diagnostics are compared with documented numerical
...
The SPS and finite-`k` RUV-III comparisons above close the absence-of-evidence
gap only for their committed synthetic cases. There is still no full PhosR
`RUVphospho` corrected-output comparison, no external missing-value comparison,
and no comparison for the historical native `sps_ruv_style` estimator. SPS
references remain required to be pre-established condition-relative log2
measurements centred on a caller-chosen reference/control baseline. SPS
discovery does not infer the baseline or convert absolute-abundance matrices.
