...
- dataset dimensions
- frame dtypes
- setup runtime and copy counts for dataset construction/request preparation
- first and repeated workflow runtime
- tracemalloc peak memory
- full-frame deep-copy counts for dataset-owned frames
- projected deep-copy counts for dataset-origin frames after safe column pruning
...
Observed local run on 2026-08-09, Windows 11
...
| Setup | 1.784228 s |
| Setup tracemalloc peak | 1.442 MiB |
| Differential first run | 0.788912 s |
| Differential repeated run | 0.357067 s |
| Differential first-run tracemalloc peak | 0.684 MiB |
| Differential repeated-run tracemalloc peak | 0.563 MiB |
| Kinase first run | 1.515288 s |
| Kinase repeated run | 1.512865 s |
| Kinase first-run tracemalloc peak | 1.250 MiB |
| Kinase repeated-run tracemalloc peak | 1.000 MiB |
| Setup full-frame deep-copy counts | `{"dataset.comparisons": 1, "dataset.phospho": 2, "dataset.sample_metadata": 1, "dataset.site_metadata": 2}` |
| Per-phase full-frame deep-copy counts | `{"differential_first": {"dataset.phospho": 1, "dataset.site_metadata": 1}, "differential_repeated": {}, "kinase_first": {"dataset.comparisons": 1, "dataset.sample_metadata": 1}, "kinase_repeated": {}, "setup": {"dataset.comparisons": 1, "dataset.phospho": 2, "dataset.sample_metadata": 1, "dataset.site_metadata": 2}}` |
| Projected deep-copy counts | `{"dataset.site_metadata projected": 2}` |
| Snapshot construction counts | `{"dataset.comparisons internal snapshot": 1, "dataset.phospho internal snapshot": 1, "dataset.sample_metadata internal snapshot": 1, "dataset.site_metadata internal snapshot": 1}` |

The copy-count observation is the relevant ownership signal: setup costs are
reported separately from workflow execution; unchanged dataset frames build one
dataset-owned immutable snapshot per accessed frame; repeated differential runs
perform one projected site-metadata ownership copy for result identity; and the
repeated kinase run performs no full dataset-frame deep copy. The runtime and
memory values are local observations for this machine and dependency set, not
...
[release-scale-builder-differential-2026-08-12.json](https://github.com/falconsmilie/phospy/blob/main/benchmarks/evidence/release-scale-builder-differential-2026-08-12.json).
...
Observed local run on 2026-08-12, Windows 11
(`Windows-11-10.0.26200-SP0`), Python 3.12.10, NumPy 2.5.1, pandas 3.0.3,
SciPy 1.18.0, and PhosPy 1.7.0:
...
| Total runtime | 299.943082 s |
| Request preparation | 0.196081 s |
| Builder execution | 284.987252 s |
| Preprocessing execution | 106.927063 s |
| Preprocessing report assembly | 0.594774 s |
| Provenance fingerprinting | 32.310527 s |
| Differential analysis | 13.484981 s |
| Result-table assembly | 0.006053 s |
| Result-table fingerprinting | 1.215470 s |
| Scientific-summary assembly | 1.228751 s |
| Process RSS peak | 652.945312 MiB |
