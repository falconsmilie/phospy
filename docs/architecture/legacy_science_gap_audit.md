...
| Motif scoring output (`motif_scores`) | Implemented as optional diagnostic output; not required in default result lane. | Implemented but not authoritative downstream | `src/phospy/prediction/motif_scoring.py`, `src/phospy/workflows/kinase/executor.py`, `tests/parity/test_prediction_science_parity.py` | Closed for supported lane |
| Profile+motif weighted combination (`combined_scores`, `weights`) | `combined_scores` is authoritative downstream; `weights` is optional diagnostic output. | Supported now | `src/phospy/prediction/scoring.py`, `src/phospy/workflows/kinase/executor.py`, `tests/parity/test_kinase_workflow_parity.py` | Closed for supported lane |
...
| Signalome network policy variants (`positive_only`, etc.) | Implemented with explicit `network_policy` (`positive_only`, `absolute_threshold`, `signed`). | Supported now | `build_kinase_network` in `src/phospy/workflows/signalome/science.py`, `SignalomeConfig` in `src/phospy/api/configs.py`, policy tests in `tests/unit/test_signalome_science.py` | `SCI-GAP-09` closed for current lane |
