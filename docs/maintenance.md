...
| Old raw-table peptide-to-site aggregation strategies | `src/phospy/science/differential/aggregation/experimental.py`, `src/phospy/science/differential/aggregation/models.py`, `src/phospy/science/differential/aggregation/executor.py` | Superseded by supported typed estimate-combination route. | Use `PeptideDifferentialEstimateTable` plus `PeptideToSiteAggregator`, or preferably resolve peptide evidence at sample-intensity level before `DifferentialAnalysisWorkflow`. | Raw `peptide_differential_table` + `evidence` aggregation now raises because it would infer uncertainty from `logFC/t`. | Retain the import route only as a delegating compatibility shell for the typed route. | `tests/unit/test_peptide_to_site_aggregation.py`. |
