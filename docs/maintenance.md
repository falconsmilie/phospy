...
| Dataset request constants `DATASET_MULTI_SITE_POLICY_*` and `DATASET_SITE_RESOLUTION_MODE_*` | `src/phospy/api/requests.py`, `src/phospy/contracts/requests.py`, `src/phospy/science/evidence/dataset_resolution/` | Public request compatibility constants. | Use the documented `DatasetBuildRequest` policy fields; constants remain valid for explicit imports. | No. | Retain; no removal target. | `tests/unit/test_public_contract_workflows.py::test_request_compatibility_constants_are_public_exports`, `tests/unit/test_dataset_peptide_evidence_resolution.py`. |
