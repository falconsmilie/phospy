...
| Old raw-table peptide-to-site aggregation strategies | `src/phospy/science/differential/aggregation/experimental.py`, `src/phospy/science/differential/aggregation/models.py`, `src/phospy/science/differential/aggregation/executor.py` | Superseded by supported typed estimate-combination route `supported_typed_estimate_combination_v2`. | Use `PeptideDifferentialEstimateTable` plus `PeptideToSiteAggregator` with explicit contrast/orientation, effect scale/unit, model/estimator, statistic distribution, uncertainty-method version, and coherent moderated-t effect/SE/statistic/p/DF rows; preferably resolve peptide evidence at sample-intensity level before `DifferentialAnalysisWorkflow`. | Raw `peptide_differential_table` + `evidence` aggregation now raises because it would infer uncertainty from `logFC/t`; post-hoc `mapping_weight` also raises because no allocation model consumes it in this lane. | Retain the import route only as a delegating compatibility shell for the typed route while the v2 identity and consistency contract remains enforced. | `tests/unit/test_peptide_to_site_aggregation.py`. |
