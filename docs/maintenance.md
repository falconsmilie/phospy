...
| `PeptideToSiteAggregator`, `PeptideToSiteAggregationConfig`, `PeptideToSiteAggregationResult`, and `PEPTIDE_TO_SITE_STRATEGY_*` | `src/phospy/science/differential/aggregation/experimental.py`, `src/phospy/science/differential/aggregation/models.py`, `src/phospy/science/differential/aggregation/executor.py` | Internal experimental compatibility only; removed from supported `phospy.science.differential` and `phospy.science.differential.aggregation` facades, and not exported from `phospy.api`. | No supported post-hoc aggregation replacement. Run differential analysis on validated analysis-ready site-level rows; do not treat same-experiment peptide-level meta-analysis as production site-level inference while the statistical model is being corrected. | `compat_best_p_value` still emits `DeprecationWarning`; the retained direct route carries `EXPERIMENTAL_INTERNAL_API=True` and `experimental_internal_compatibility_only` status. | Retain only as internal compatibility until a corrected model, ADR, public contract, docs, and tests justify future support; otherwise remove. | `tests/unit/test_peptide_to_site_aggregation.py::test_peptide_to_site_aggregation_absent_from_supported_public_facades`, `tests/unit/test_peptide_to_site_aggregation.py::test_experimental_compatibility_route_is_explicitly_internal`, `tests/unit/test_peptide_to_site_aggregation.py::test_compatibility_mode_reproduces_minimum_p_value_selection`. |
