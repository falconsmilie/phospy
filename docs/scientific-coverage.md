...
The experimental protein-aware differential lane is selected only through
`DifferentialAnalysisConfig.protein_aware_model` with method
`protein_covariate_adjusted_moderated_linear_model_v1`. It consumes the current
dataset's `ProteinAwarePreparationResult`, fits the requested phosphosite
condition contrast conditional on the matched measured total-protein covariate,
centres but does not standardize or impute the protein covariate, and keeps
withheld rows visible in the full result index. It is not phosphosite/protein
subtraction, stoichiometry, occupancy, causal decomposition, MSstatsPTM parity,
or MSstatsPTM-style joint PTM/protein inference. Ordinary differential analysis
ignores a protein-aware sidecar when the opt-in config is absent.
...
- `SpsRuvBatchCorrectionConfig`, a native SPS/RUV-style preprocessing
  correction lane with separate `sps_ruv_style` and `ruv_iii_style` methods
  that requires caller-supplied controls, protected design
  ...
  `sps_ruv_style` rejects actual missing values at execution. `ruv_iii_style`
  can use row-median completion internally and restores actual missing positions
  afterward; analysis-ready construction still requires upstream preprocessing
  to provide a complete numeric matrix. Upstream-imputed cells remain tracked through
...
- Fixture-scoped external comparison of SPS discovery against pinned PhosR
  ...
  condition-relative log2 SPS matrices and already selected RUV controls.
  This evidence does not extend to the complete `RUVphospho` workflow, the
  historical `sps_ruv_style` estimator, or missing-value execution.
...
- no claim of broad native `ruv_iii_style` or `RUVphospho` workflow parity
  beyond the committed complete-input finite-`k` kernel cases.
...
- `benchmark_evidence`: dated machine-specific runtime, memory, environment,
  and fingerprint evidence for an executed workload. This is operational scale
  evidence only, not scientific validation, external parity, or a portable
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Differential analysis | Contrast-level site log-fold change, moderated statistics, p-values, adjusted p-values, tested/withheld row status, and optional depth-aware prior/posterior variance moderation | Analysis-ready complete site matrix; explicit `ExperimentalDesign`; explicit `Contrast`; empirical-Bayes config; optional feature-level `site_metadata["quantification_depth"]` with declared depth kind | R/limma fixtures for the documented two-condition and large-trend lanes; R/DEqMS `spectraCounteBayes` fixture for count-aware depth moderation; PhosPy-owned adverse-design fixtures for unsupported/edge designs | Exact/tolerance checks for OLS coefficients where equivalent; documented drift envelopes for moderated trend quantities; explicit row-status checks for withheld rows; DEqMS depth fixture compares prior variance, prior degrees of freedom, posterior variance, moderated t, raw p-value, and adjusted p-value with explicit correlation, log-difference, and absolute-difference thresholds | Large-feature trend branch, lower- and higher-depth sites, an otherwise identical low/high-depth pair, unbalanced condition groups, fixed covariates, fixed-block completeness, non-estimable contrasts, imputation-withheld rows | R/limma and R/DEqMS manifests under `tests/fixtures/rewrite_parity/differential_*` are `parity`; adverse-design fixtures/tests are `regression`; no empirical validation claimed | Fixture-scoped limma parity for named envelopes; depth-aware moderation is validated as quantification-depth-aware empirical Bayes moderation inspired by DEqMS, not exact DEqMS-compatible numerical equivalence; broader differential designs are validated PhosPy contracts or open gaps only |
| Protein-aware differential analysis | Condition-level phosphosite log-fold change conditional on the matched measured total-protein abundance covariate | Analysis-ready established-log2 phosphosite matrix; established-log2 total-protein matrix; dataset-owned `ProteinAwarePreparationResult`; explicit `ExperimentalDesign`; explicit `Contrast`; `protein_covariate_adjusted_moderated_linear_model_v1` opt-in | Independent linear-algebra oracle and synthetic scenarios in the protein-aware differential science tests; no external MSstatsPTM or limma protein-aware parity fixture | Grouped augmented-design coefficient, residual-variance, moderation-family, multiple-testing-family, full-index status, and provenance/fingerprint assertions | Multiple protein-row groups, sample subset/reorder, fixed covariates, fixed-block terms, missing sidecar rejection, duplicate-correlation rejection, technical-aggregation rejection, covariate zero-variance and non-finite withholding | `tests/science/test_differential_protein_aware_scientific_validation.py` is `synthetic_validation`; unit and integration protein-aware differential tests are `regression`; no external parity or empirical validation claimed | Experimental PhosPy estimator only. The reported condition effect is conditional on measured matched total protein; it is not stoichiometry, occupancy, causal decomposition, MSstatsPTM parity, or MSstatsPTM-style joint PTM/protein inference |
...
| SPS discovery and finite-`k` RUV-III external comparison | SPS per-reference stability, consensus ranking/top-N selection, and RUV-III corrected matrices/diagnostics | Three synthetic reference datasets explicitly prepared as baseline-centred condition-relative log2 matrices; replicated conditions; 210 shared sites; complete RUV matrix; four already selected negative controls; four replicate sets; `k=0,1,2` | Pinned R 4.5.2, PhosR 1.13.1 commit `1be74902b775833c64f5833e70538eaf843cf6a5`, and `ruv` 0.9.7.1 outputs under `tests/fixtures/rewrite_parity/phosr_ruv/` | SPS scalar intermediates within `1e-12`, exact top-N outside external tie groups, RUV corrected matrices at `atol=1e-10`/`rtol=1e-12`, diagnostics within `1e-10` | Different SPS input row orders, stable/strongly changing sites, tie group, requested top-N, RUV `k` values, two unwanted factors, retained biological effects, sample-order permutation | Manifest-governed `parity`; deterministic synthetic inputs; no external biological dataset; expected outputs serialized from pinned R packages; no empirical validation | Fixture-scoped external agreement only. Partial-reference SPS contributions, ambiguous absolute-abundance inputs, missing-value RUV execution, native `sps_ruv_style`, and the full PhosR `RUVphospho` workflow remain outside the claim |
| Peptide-to-site aggregation | Site-level sample abundance after peptide evidence allocation before differential modelling | Peptide evidence rows, sample intensity columns, explicit site mapping or site strings, multi-site policy, localisation confidence when supplied | Raw synthetic peptide-evidence fixtures, explicit mappings, declared known truth, and independently generated expected estimator output in `tests/fixtures/release_validation_regression/peptide_site_bias_regimes/`; deterministic resolver fixtures in `evidence_resolution/` | Raw fixtures are executed through `PeptideEvidenceDatasetResolver`; production output must match the independent expected site-estimate matrix within `1e-12`; signed and absolute bias against declared known truth is reported by regime; precomputed bias summary remains fixture-integrity/reporting evidence only | Duplicate concordant control, duplicate discordance, equal ambiguous splitting, unequal explicit fractions, missing value in one sample, entirely missing peptide observation, localisation-error signal transfer, row-order invariance, sequence conflicts | Peptide bias fixtures are production-path `synthetic_validation`; resolver expected outputs in `evidence_resolution/` are PhosPy-owned `regression`; no external parity or empirical validation claimed | The tested estimator is linear peptide-abundance fractional allocation followed by finite-value arithmetic mean per site/sample under explicit mapping weights; the supported sample-intensity route is auditable and adverse-regime bias is quantified; post-hoc peptide differential estimate combination remains withdrawn |
...
| Release-scale builder and differential workflow | Operational scale evidence for public builder, preprocessing, provenance/fingerprinting, and one differential contrast over 50,000 sites x 48 samples | Deterministic generated 50,000 x 48 workload with site/sample metadata, log2 transform, median centering, row-median imputation, and one two-condition contrast | No external scientific reference; benchmark output fingerprints and invariants from the same run | Benchmark report schema requires Python, dependency, machine, runtime, peak-memory, metrics, scientific summary, output fingerprints, and source provenance; workflow invariants must pass before report write | Machine-dependent local runtime/RSS; deterministic output fingerprints; preprocessing-stage sequence and result dimensions | The retained report [release-scale-builder-differential-2026-08-12.json](https://github.com/falconsmilie/phospy/blob/main/benchmarks/evidence/release-scale-builder-differential-2026-08-12.json) is `benchmark_evidence` only; it is generated by explicit `benchmarks/measure_release_scale_builder_differential.py --report-path benchmarks/evidence/<dated-file>.json` invocation and the workload is excluded from CI, release gates, scheduled jobs, and per-commit checks | Production-scale feasibility evidence for one local environment, not a portable runtime guarantee, scientific validation, or scientific parity |
...
| --- | --- | --- | --- | --- |
| Differential analysis | `parity-gated` | `DifferentialAnalysisWorkflow` for two-condition unpaired simple contrasts with empirical-Bayes `standard`/`robust` and optional `trend` over mean intensity or declared quantification depth; fixed-effect batch, categorical covariate, continuous covariate, complete fixed-block terms, and explicit duplicate-correlation paired designs are executable. `duplicate_correlation` estimates one REML consensus correlation and fits compound-symmetry GLS. Upstream-imputed datasets remain rejected by default; `imputed_value_policy="withhold_imputed_features"` is an explicit validated PhosPy policy when imputation observation metadata is present. | `tests/parity/test_differential_analysis_parity.py`, `tests/parity/test_differential_limma_parity.py`, `tests/parity/test_differential_limma_trend_large.py`, `tests/parity/test_differential_deqms_depth_parity.py`, `tests/fixtures/rewrite_parity/differential_duplicate_correlation/`, `tests/fixtures/rewrite_parity/differential_deqms_depth/`, `tests/science/test_differential_adverse_design_contracts.py`, plus unit/integration design, fixed-effect provenance, result-contract tests, and `tests/unit/test_differential_imputation_policy.py` | Limma parity is fixture-scoped, including the 1,600-feature trend fixture and duplicate-correlation fixtures. The DEqMS depth fixture validates count-aware shrinkage behavior for lower- and higher-depth sites and an otherwise identical low/high-depth pair, but PhosPy keeps its existing trend smoother; describe the feature as quantification-depth-aware empirical Bayes moderation inspired by DEqMS, not exact DEqMS-compatible numerical equivalence. Adverse fixed-effect/design cases without external expected values are PhosPy regression contracts, not parity. Fixed-effect batch terms are not batch correction. Fixed-block terms require complete within-block contrast coverage and full-rank/estimable designs. `duplicate_correlation` is one consensus compound-symmetry correlation, not feature-specific random effects. Mixed-effect, random-slope, multi-random-effect, and random subject-effect designs are rejected in this release. Missing values are rejected at analysis-ready boundary before model fitting. Imputed cells are not treated as fully observed by default. `withhold_imputed_features` withholds high-imputation or insufficient-observation rows, reports status, and excludes withheld rows from model fitting and the Benjamini-Hochberg denominator. It is not observed-only fitting. |
| Protein-aware differential analysis: `protein_covariate_adjusted_moderated_linear_model_v1` | `experimental` | Opt-in `DifferentialAnalysisWorkflow` lane selected by `DifferentialAnalysisConfig.protein_aware_model`. The workflow consumes only the current dataset's `ProteinAwarePreparationResult`, fits `y_s = X beta_s + z_p(s) gamma_s + error` by total-protein-row groups, centres the matched protein covariate without standardizing or imputing it, and returns ordinary `DifferentialAnalysisResult` tables with typed full-index attrition, protein-aware diagnostics, caveats, and provenance. The ordinary default is unchanged when the option is absent. | `tests/unit/test_differential_protein_aware_contracts.py`, `tests/unit/test_differential_protein_aware_result_assembly.py`, `tests/integration/test_differential_protein_aware_workflow.py`, and `tests/science/test_differential_protein_aware_scientific_validation.py` | Experimental PhosPy estimator only. The condition `logFC` is conditional on measured matched total-protein abundance. It is not phosphosite/protein subtraction, stoichiometry, occupancy, causal separation, MSstatsPTM parity, or MSstatsPTM-style joint PTM/protein inference. `duplicate_correlation`, actual technical-replicate aggregation, mixed effects, protein normalization, protein imputation, and ordinary-lane fallback are unsupported. |
...
| Batch correction: `SpsRuvBatchCorrectionConfig` | `validated PhosPy implementation`; fixture-scoped external comparison for SPS discovery and complete-input finite-`k` RUV-III | Dataset preprocessing supports two explicit native control-site correction methods. `sps_ruv_style` retains protected-design estimation and provenance-only optional replicate metadata. `ruv_iii_style` requires replicate metadata and uses its replicate sets directly in the RUV-III calculation. Both consume `ControlSiteSet`, including sets returned by SPS discovery, without reference-matrix reconstruction. Both preserve observation-mask provenance; RUV-III internal row-median completion restores actual missing positions, while analysis-ready construction continues to require complete numeric output. | Unit config/validator and numerical-kernel tests, SPS/RUV regression tests, workflow orchestration/provenance tests, dataset integration tests including direct discovery-control consumption, and pinned external evidence in `tests/parity/test_phosr_sps_ruv_parity.py` plus `tests/fixtures/rewrite_parity/phosr_ruv/` | These are native PhosPy methods, not broad PhosR-equivalent SPS/RUV-III parity, ComBat, or limma `removeBatchEffect`. External agreement is limited to the committed SPS and complete-input finite-`k` RUV-III cases; it excludes full `RUVphospho`, missing values, and `sps_ruv_style`. The two native methods have intentionally different replicate and protected-structure semantics. Controls remain explicit governed annotations; SPS discovery stays separate from correction execution. Unsupported stage orders and temporary-imputation methods are rejected. |
| Joint PTM/protein modelling and MSstatsPTM-style inference | `open gap` | No executable MSstatsPTM-style separate PTM/protein model-output workflow is supported. The only executable protein+phosphosite differential lane is the separately listed experimental version-1 matched-covariate estimator; broader joint/separate-model adjusted inference remains an open gap. | N/A | Do not interpret total-protein subtraction, protein-aware preparation, or `protein_covariate_adjusted_moderated_linear_model_v1` as MSstatsPTM-style inference, MSstatsPTM parity, stoichiometry, occupancy, or equivalence to MSstatsPTM. |
| PhosR-equivalent SPS/RUV-III, ComBat, and `removeBatchEffect` parity | `open gap` | No broad PhosR-equivalent SPS/RUV-III, ComBat, or limma `removeBatchEffect` parity lane is supported. `ruv_readiness` is report-only RUV-readiness metadata. | Narrow pinned SPS and finite-`k` RUV-III external fixtures exist; there is no full `RUVphospho`, ComBat, or `removeBatchEffect` correction parity claim | Do not interpret fixture-scoped SPS/RUV-III kernel evidence or report-only `ruv_readiness` metadata as broad RUV support. Do not interpret `linear_residualize_batch` fixed-effect residualisation as ComBat, RUV, SPS/RUV-III, limma `removeBatchEffect`, or mixed-effects support. Do not interpret native SPS/RUV-style correction as blanket PhosR parity. |
...
- CI may validate the retained release-scale benchmark JSON schema, but it must
  not rerun the 50,000 x 48 workload.
- CI parity workflows:
...
- In `KinaseWorkflow`, `scoring_mode="kinase_library_contextual_motif"` still runs inside
...
- Protein-aware preparation is preparation-only model-input preparation. It
  ...
  not adjust differential models by itself, and does not claim
  MSstatsPTM-style inference or equivalence.
- `protein_covariate_adjusted_moderated_linear_model_v1` is an experimental
  opt-in differential estimator. It consumes only the current dataset's
  protein-aware preparation sidecar, centres but does not standardize or impute
  the matched total-protein covariate, withholds inadmissible rows without
  ordinary-lane fallback, and reports a condition effect conditional on measured
  total-protein abundance. It is not stoichiometry, occupancy, causal
  decomposition, MSstatsPTM parity, or MSstatsPTM-style joint PTM/protein
  inference.
...
- Enrichment ORA results must not be interpreted as GSEA, ssGSEA, or PTM-SEA
  support.
- MSstatsPTM-style joint/separate-model PTM/protein adjusted inference is not
...
- protein-aware estimator provenance when selected, including method ID,
  experimental claim status, model formula, conditional `logFC` interpretation,
  preparation/mapping policies, execution sample order, no-standardization,
  no-imputation and no-fallback policies, typed row counts, augmented-design
  condition-number summaries, input fingerprints, limitations, and unsupported
  claims
...
provenance without implying a downstream model by itself.
...
PTM/protein differential modelling by itself, and does not claim MSstatsPTM
equivalence.
