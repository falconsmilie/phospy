...
redistribution caveats; those caveats must not be replaced with optimistic
wording. Bundled release eligibility is governed by `redistribution_status`,
not by broad source-name assumptions. The release gate rejects bundled
manifests that are missing required metadata, omit packaged files, fail file
hash verification, or declare a non-release-eligible status. Unresolved bundled
references block release. External-only references must remain caller-supplied
and must not be shipped as bundled data. Codex agents and human developers must
not mark references `approved` without verified evidence in the manifest for
the exact files being packaged.
...
| --- | --- | --- |
| References | Bundled runtime references are rat-only. The packaged rat lane has a hash-verifiable manifest and explicit provenance caveats. Human and mouse workflows require an explicit caller-supplied `ReferenceBundle`. | Broader reference handling should use explicit provenance, compatibility checks, external bundle validation, and file-level hashes. New bundled data requires verified redistribution permission, provenance, docs, and tests before `_BUNDLED_DEFAULTS` is updated. Packaged manifests with `unresolved` or `external_only` redistribution status block release. |
