...
| Phosphosite representation | Site-centric workflows with explicit phosphosite IDs and sequence context used downstream. | Canonical site ID contract with strict site metadata and boundary validation. | partially supported | Required parity | scientific gap | high | Maintain current site-ID contract and add/keep fixture-backed comparisons for site identity-sensitive lanes. | `docs/adr/adr_0018_phosphosite_identity_and_localisation_policy.md`; `tests/unit/test_site_identifier_canonicalization.py`; `tests/parity/test_preprocessing_science_parity.py` | PhosR vignette; ADR-0018 references |
