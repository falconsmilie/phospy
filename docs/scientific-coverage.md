...
- `SpsRuvBatchCorrectionConfig`, a native SPS/RUV-style preprocessing
  ...
  limited to `none` and `row_median_temporary`, but the public native workflow
  requires a complete correction-stage matrix and rejects actual missing values
  (NaNs) before executor invocation. `row_median_temporary` is a recognized
  policy/mechanics label, not permission for actual NaNs to pass through the
  public native workflow. Upstream-imputed cells remain tracked through
