...
| Peptide-to-site differential evidence | `validated PhosPy implementation` | Preferred lane resolves peptide evidence at sample-intensity level before differential model fitting during dataset building before `DifferentialAnalysisWorkflow`. Advanced post-hoc `PeptideToSiteAggregator` support requires `PeptideDifferentialEstimateTable` typed contrast identity, orientation, effect scale/unit, model/estimator, statistic distribution, uncertainty-method version, effect, standard error, original p-value, residual/moderated degrees of freedom, source experiment/run identifier, dependence policy, and mapping policy. Moderated-t rows must pass effect/statistic sign, effect/standard-error/statistic, and p/statistic/DF consistency checks. | `tests/unit/test_peptide_to_site_aggregation.py`, dataset peptide-evidence resolution tests, public export tests, documentation scope tests, and deterministic provenance tests. | Single-estimate outputs are pass-through and are not called meta-analysis. Multi-estimate post-hoc support is limited to independent source experiments/runs with one comparable estimate identity per aggregation run. Same-experiment peptide estimates are rejected because same-sample peptide dependence is not modelled. Stouffer z combination uses signed two-sided p-value conversion, not `z=t`. Fixed-effect inverse-variance requires the documented large-DF asymptotic-normal eligibility policy. `mapping_weight` is rejected in the post-hoc lane. Multiple testing uses the configured shared correction method. |
...
- peptide-to-site differential estimate combination:
...
- What it does:
  records execution of the supported
  `supported_typed_estimate_combination_v2` peptide-to-site post-hoc
  differential estimate combination, while documenting that the preferred
  PhosPy-origin lane resolves peptide evidence at the sample-intensity level
  before differential fitting.
- Assumptions:
  single-estimate site rows pass through the original effect, statistic, and
  finite-df p-value; multi-estimate rows require independent source
  experiments/runs; same-experiment peptide dependence is rejected rather than
  treated as independent; each aggregation run has one comparable contrast,
  orientation, effect scale/unit, model/estimator, statistic distribution, and
  uncertainty-method version.
- Parameters:
  support status, uncertainty method, minimum estimates per site, dependence
  policy, multiple-testing method, input contrast identity, effect scale,
  model/estimator, statistic distribution, consistency policy and tolerance
  version, approximation policy, mapping-weight policy, input mapping policies,
  and signed finite-df t-to-z conversion policy.
- Output meaning:
  explicit provenance for a typed site-level post-hoc differential estimate
  summary under the recorded dependence and uncertainty assumptions.
