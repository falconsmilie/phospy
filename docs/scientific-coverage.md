...
- no claim that native `ruv_iii_style` provides PhosR numerical parity.
...
| Batch correction: `SpsRuvBatchCorrectionConfig` | `validated PhosPy implementation` | Dataset preprocessing supports two explicit native control-site correction methods. `sps_ruv_style` retains protected-design estimation and provenance-only optional replicate metadata. `ruv_iii_style` requires replicate metadata and uses its replicate sets directly in the RUV-III calculation. Both consume `ControlSiteSet`, including sets returned by SPS discovery, without reference-matrix reconstruction. Both preserve observation-mask provenance; RUV-III internal row-median completion restores actual missing positions, while analysis-ready construction continues to require complete numeric output. | Unit config/validator and numerical-kernel tests, SPS/RUV regression tests, workflow orchestration/provenance tests, and dataset integration tests including direct discovery-control consumption | These are native PhosPy methods, not PhosR-equivalent SPS/RUV-III parity, ComBat, or limma `removeBatchEffect`. `sps_ruv_style` and `ruv_iii_style` have intentionally different replicate and protected-structure semantics. Controls remain explicit governed annotations; SPS discovery stays separate from correction execution. Unsupported stage orders and temporary-imputation methods are rejected. |
