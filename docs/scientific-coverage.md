...
| Replicate/condition modelling | `PHOSPY_VALIDATED_SCIENCE` | `required parity` | Typed `ExperimentalDesign` and `Contrast` contract for differential workflows | No implicit condition inference from sample names | Contract and boundary tests for design/contrast validation (`tests/unit/test_experimental_design_contract.py`, `tests/unit/test_differential_analysis.py`) |
