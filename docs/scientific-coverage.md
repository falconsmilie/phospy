...
| Peptide-to-site differential evidence | `validated PhosPy implementation` | Preferred lane resolves peptide evidence at sample-intensity level before differential model fitting during dataset building before `DifferentialAnalysisWorkflow`. Advanced post-hoc `PeptideToSiteAggregator` support requires `PeptideDifferentialEstimateTable` typed effect, standard error, original p-value, residual/moderated degrees of freedom, source experiment/run id, dependence policy, and mapping policy. | `tests/unit/test_peptide_to_site_aggregation.py`, dataset peptide-evidence resolution tests, public export tests, documentation scope tests, and deterministic provenance tests. | Single-estimate outputs are pass-through and are not called meta-analysis. Multi-estimate post-hoc support is limited to independent source experiments/runs. Same-experiment peptide estimates are rejected because same-sample peptide dependence is not modelled. Stouffer z combination uses signed two-sided p-value conversion, not `z=t`. Multiple testing uses the configured shared correction method. |
...
- What it does:
  records execution of typed peptide-to-site post-hoc differential estimate
  combination, while documenting that the preferred PhosPy-origin lane resolves
  peptide evidence at the sample-intensity level before differential fitting.
- Assumptions:
...
- Parameters:
  ...
  policy, multiple-testing method, input mapping policies, and signed
  finite-df t-to-z conversion policy.
- Output meaning:
